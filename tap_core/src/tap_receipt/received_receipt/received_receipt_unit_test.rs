...
use crate::{
    ...
    tap_receipt::{get_full_list_of_checks, Receipt, ReceiptCheckResults, ReceivedReceipt},
};
...
fn receipt_adapters() -> (ReceiptStorageAdapterMock, Arc<RwLock<HashMap<u64, u128>>>) {
    ...
    (receipt_storage_adapter, query_appraisal_storage)
}
...
async fn partial_then_full_check_valid_receipt(
    ...
    let (_, query_appraisal_storage) = receipt_adapters;
    let (_escrow_adapter, escrow_storage) = escrow_adapters;
